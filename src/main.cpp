...
#if defined(NDEBUG)
# error "Dcscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4b00545b94963bf6d05765ec21ebe69a100c48ec9ed2e8ca52cc0b5a13c3e85b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dcscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dcscoin Signed Message:\n";
...
{
    // Dcscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 400000); // Dcscoin: 400k blocks in ~4 years
    // Simply means that after 400k blocks are mined the reward value will be cut and half 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Dcscoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Dcscoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dcscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dcscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcc820ae93fdfbe0244d6a38ded50049fbf39dc663074bba983e24000efffc6f3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 19/04/2020 - Covid-19 is taking life out of us!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046419b946aea04b826c02ee1ac5d51707dd8b2c0ba3815ab53db6de3b5f84026d4953a5f80f9446d83e042125169976d5e36e1e080f305b53eba93c1f320cfcd0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1587274754;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085217441;

        if (fTestNet)
        {
            block.nTime    = 1587274754;
            block.nNonce   = 385841801;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd105d1cfd8cd5668f179acdf6f0255dbbcbdbf2d228d025d32dac99e500bc3f8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb3, 0xdc }; // Dcscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DcscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DcscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DcscoinMiner(CWallet *pwallet)
{
    printf("DcscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dcscoin-miner");
    ...
    try { loop {
        ...
        printf("Running DcscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DcscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DcscoinMiner, pwallet));
}
